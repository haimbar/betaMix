...
1. **Fitted mixture** — the standard histogram overlay with null component
   ...
   significance region.
2. **Null Q-Q plot** — hard-assigned null observations ($m_0 \geq 0.5$) vs
   $\text{Beta}(\hat{\eta}, 0.5)$. Points are blue when the KS $D$ statistic
   is $\leq 0.05$ (good fit) and red otherwise. A notable departure from the
   diagonal may indicate dependent samples; try `ind = FALSE` in that case.
3. **Non-null Q-Q plot** — hard-assigned non-null observations ($m_0 < 0.5$),
   scaled by $b_{\max}$, vs $\text{Beta}(\hat{a}, \hat{b})$. Systematic
   curvature or red points suggest the non-null component is not well described
   by a single Beta distribution.
...
The printed assessment reports KS $D$ for both components, the number of
detected modes, and an overall verdict (`"Good fit"`, `"Acceptable fit"`, or
`"Deviations detected"`). The D statistic threshold is 0.05; raw p-values
are not used because with tens of thousands of pairs even $D \approx 0.01$
yields $p \approx 0$.
