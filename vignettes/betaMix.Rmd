...
4. **Non-null kernel density** — the empirical density (blue) overlaid with
...
The current threshold is set by `maxalpha` and `ppr`, which control Type I
error and the posterior probability rate respectively. After fitting,
`plotROC()` provides a complementary data-driven view: it sweeps $\tau$ from
0 to 1 and plots the exact false- and true-positive rates implied by the
fitted Beta distributions — no ground-truth labels are needed.
